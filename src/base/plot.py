from sklearn.metrics import confusion_matrix, classification_report, r2_score, mean_squared_error, mean_absolute_percentage_error
import numpy as np
...
#     precision_fold, recall_fold, f1_score_fold, support_fold = precision_recall_fscore_support(df.Class, df.Pred)
#     metrics_fold = {'precision':precision_fold, 'recall':recall_fold, 'f1-score':f1_score_fold, 'support':support_fold}
    if labels is None:
        labels = [new.trans[i] for i in range(new.num_classes)]
#     cm_fold = confusion_matrix(df.Class.map(new.trans_inv), df.Pred.map(new.trans_inv), labels = np.arange(new.num_classes), normalize='true')
...
def obtain_accumulated_metrics(reports_, metric = 'mean', label_order=None):
    reports_np =100*np.array( [pd.DataFrame(i)[label_order].loc[['precision', 'recall','f1-score','support']] for i in reports_])
    reports_df =[100*pd.DataFrame(i)[label_order].loc[['precision', 'recall','f1-score','support']] for i in reports_]
...
def plot_confusion_matrix(cm_folds,
    ...
    for i, j in itertools.product(range(num_classes), range(num_classes)):
        if center_cm[i, j]>=1e-1:
            ...
            sup_text = plt.text(new_coords[1,0]-0.115, new_coords[1,1]-0.01, super_text, verticalalignment = 'bottom', horizontalalignment="left", fontdict={'fontsize':SMALL_SIZE-3}
                 ,color="white" if center_cm[i, j] > 80 else "black")
