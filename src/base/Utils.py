...
def read(df, cols, lc_params, n_bands):
    ...
    # DT = lc[:,1] - shift(lc[:,1], [1], cval=0)
    # df_lc = df_lc.assign(DT=DT)
    # lc = np.hstack((lc, DT.reshape(-1,1)))
    ...
    for band in range(n_bands):
        ...
        lc_band = lc[b][:,[1,2,3,4]]#.astype(np.float32)
    ...
    if len(phys_params_est) > 0:
        # phys_params_est = [df[i] for i in phys_params_est]
...
def process_lc(cls_,
    ...
    for b in range(n_bands):
        ...
        # print(b)
        # print(np.any(np.isnan(matrices[b])))
        # print(matrices[b])
    ...
    out_dict = {}
    out_dict['Label'] =cls_
    out_dict['Order'] =orders
    out_dict['ID'] =ID
    out_dict['Physical_Values'] =phys_values
    out_dict['Estimated_Physical_Values'] =phys_values_est
    out_dict['Matrices'] = matrices
    out_dict['M_0'] = M0
    out_dict['T_0'] = T0
...
def serialize(dict,
    ...
    n_bands = len(dict['M_0'])

    dict_features={
    'ID': tf.train.Feature(bytes_list=tf.train.BytesList(value=[str(dict['ID']).encode()])),
    'Label': tf.train.Feature(int64_list=tf.train.Int64List(value=[dict['Label']])),
    }

    # Add the physical values
    for key in dict['Physical_Values'].keys():
        dict_features[key] = tf.train.Feature(float_list=tf.train.FloatList(value=[dict['Physical_Values'][key]]))
    # Add the estimation of the physical valeus
    for key in dict['Estimated_Physical_Values'].keys():
        dict_features[key] = tf.train.Feature(float_list=tf.train.FloatList(value=[dict['Estimated_Physical_Values'][key]]))

    # Add contextual information per band
    for i in range(n_bands):
        # Length of each band
        dict_features["N_"+str(i)] = tf.train.Feature(
            int64_list = tf.train.Int64List(value=[dict['Matrices'][i].shape[0]]))
        # First magnitude
        dict_features["M0_"+str(i)] = tf.train.Feature(
            float_list = tf.train.FloatList(value=[dict['M_0'][i]]))
        # First time
        dict_features["T0_"+str(i)] = tf.train.Feature(
            float_list = tf.train.FloatList(value=[dict['T_0'][i]]))
    ...
    for i in range(n_bands):
        lc_ = tf.train.Feature(
                    float_list=tf.train.FloatList(
                                        value=dict['Matrices'][i].ravel()))
        o_ = tf.train.Feature(
                    int64_list=tf.train.Int64List(
                                        value=dict['Order'][i].ravel()))
        u_ = tf.train.Feature(
                    float_list=tf.train.FloatList(
                                        value=dict['Uncertainty'][i].ravel()))
