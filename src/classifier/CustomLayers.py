...
class RNNLayersBands(tf.keras.Model):
    def __init__(self,
        ...
        # Create the proyections of the residual connection
        if self.bidirectional:
            self.proyection_W_f = tf.keras.layers.Dense(rnns[0][0].cell.units, name='Res_W_b_0')
            ...
            self.proyection_W_f = tf.keras.layers.Dense(rnns[0].cell.units, name='Res_W_b_0')
    ...
    def call_one(self, rnns, input_, N_, backwards = False):
        ...
        if not backwards:
            proyection_W = self.proyection_W_f
