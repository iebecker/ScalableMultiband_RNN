4mport matplotlib.pyplot as plt
import os
import tensorflow as tf
import BaseClass.Multiband_05 as Multiband
from pandas import DataFrame
import numpy as np
import json
from Classifier.Model_54.CustomModels_v4 import CustomModelBand, CustomModelCentral
from Classifier.Model_54.CustomMetrics import CustomAccuracy, CustomTopKAccuracy, CustomFinalAccuracy, CustomTopKFinalAccuracy
from Classifier.Model_54.CustomMetrics import Masked_RMSE, CustomFinalF1Score, Masked_R2
from Classifier.Model_54.CustomLayers import InputCentral, SauceLayer, ApplyMask, LastRelevantLayer, MeanMagLayer
from Classifier.Model_54.CustomLayers import RNNLayersCentral, RNNLayersBands, MeanColorLayer, PhysicalParamsLayer
from Classifier.Model_54.CustomLayers import AllTimes, RawTimesLayer
from Classifier.Model_54.CustomLosses import CrossEntropy_FullWeights, CategoricalFocalLoss, MSE_masked
import BaseClass.Parser_05 as Parser
...
class Network(Multiband.Network):
    ...
    def __add_model_band(self, i):
        ...
        MeanMag = MeanMagLayer(self.w,
                               name='MeanMag_'+str(i)
                               )
        mean_mags = MeanMag(self.inputs['input_LC_'+str(i)],
        ...
        # Get proyect to num_Classes dimensions
        proyections = self.sauce[i]
        for j in range(len(self.fc_layers_bands)):
            # Traditional BatchNorm
            proyections = tf.keras.layers.Dense(self.fc_layers_bands[j],
                                                # activation='relu',
                                                activation=None,
                                                use_bias=False,
                                                )(proyections)
            ...
            proyections = tf.keras.layers.Dropout(self.dropout)(proyections)
        # Add softmax layer
        ...
        self.train_metrics[i] = {'Class': [CustomAccuracy(name='Acc', N_skip=self.N_skip, mask_value = self.mask_value),
                                           CustomTopKAccuracy(k=2, name='Top2', N_skip=self.N_skip, mask_value = self.mask_value),]
                                 }
    ...
    def __add_model_central(self):
        OutputsBands = [None]*self.n_bands
        Sauce_Layers = [None]*self.n_bands
        Sauce = [None]*self.n_bands
        orders = [None]*self.n_bands
        outs = [None]*self.n_bands
        MeanMags = [None]*self.n_bands
        RawTimes = [None]*self.n_bands
        ...
        for j in range(self.n_bands):
          ...
          # Get output proyections
          orders[j] = self.inputs_central['O_'+str(j)]

          # Get the sauce
          OutputsBands[j] = outs[j]['Sauce_'+str(j)]

          # Get the MeanMag per timestep per band
          MeanMags[j] = outs[j]['MeanMag_'+str(j)]

          # Get the raw time per timestep per band
          RawTimes[j] = outs[j]['RawTimes_'+str(j)]

          # Concatenate the raw input to each band
          if self.use_raw_input_central:
              Sauce_Layers[j] = SauceLayer(len(OutputsBands[j])+1, name='SauceLayer_Central_'+str(j))
              proyection_RawInput= tf.keras.layers.Dense(self.size_hidden_bands[0])
              proyected_RawInput = proyection_RawInput(self.inputs_central['input_LC_'+str(j)])

              Sauce[j] = Sauce_Layers[j]([proyected_RawInput]+OutputsBands[j])
          else: # Do not use raw input
              # Give control of the weighting to the spine
              Sauce_Layers[j] = SauceLayer(len(OutputsBands[j]), name='SauceLayer_Central_'+str(j))
        ...
        InputCentralLayer = InputCentral(name='Input_Central')

        Ns = [self.inputs_central['N_'+str(i)] for i in range(self.n_bands)]
        sorted_states, N_central = InputCentralLayer(Sauce, orders, Ns)

        # Compute the colors per time-step
        MeanColors = MeanColorLayer(self.n_bands, name='Mean_Color')
        colors = MeanColors(MeanMags, Ns, orders)

        # Compute the deltas over the entire time
        ComputeDeltas = AllTimes(self.n_bands, name='AllTimes_Central')
        DeltaTimes = ComputeDeltas(RawTimes, N_central)
        ...
        for k in range(len(self.fc_layers_central)):
            ...
            # dense_central = tf.keras.activations.relu(dense_central)
            # CHANGE OP TO LAYER
            # dense_central = tf.keras.layers.Dropout(self.dropout)(dense_central)
        ...
        ApplyMasks = ApplyMask(self.num_classes,
                               mask_value = self.mask_value,
                               name='Class')
        masked_prediction_central = ApplyMasks(prediction_central, N_central)

        # Get last prediction
        LastRelevant = LastRelevantLayer(name='FinalClass')
        last_prediction_central = LastRelevant(prediction_central, N_central)
        ...
        for param in self.physical_params:
            ...
            lastPhys[param] = tf.keras.layers.Dense(1,
                                                    activation=None,
                                                    use_bias=True,
        ...
        for param in self.physical_params:
            self.outputs_end[param] = lastPhys[param]
        ...
        self.train_metrics['Central'] = {'Class': [CustomAccuracy(name='CentralAcc', N_skip=self.N_skip, mask_value = self.mask_value),
    ...
    def train_loop(self):
        ...
        try:
            ...
            for epoch in range(self.epochs):
                self.epoch = epoch
                for input, target in self.dataset_train:
                    self.step = batch
                    # Run the train step
                    self.train_logic(input, target)
                    batch+=1
    ...
    def val_loop(self):
        ...
        with self.train_summary_writer_C.as_default():
            ...
            tf.summary.scalar('Classifier_FinalTop2Accuracy', self.log_train_central['Class_FinalTop2'], step=self.epoch)
            ...
            for param in self.physical_params:
                tf.summary.scalar('Regression_Masked_RMSE_'+param, self.log_train_central['Pred_'+param+'_'+'Masked_RMSE'], step=self.epoch)
                tf.summary.scalar('Regression_Masked_R2_'+param, self.log_train_central['Pred_'+param+'_'+'Masked_R2'], step=self.epoch)
        ...
        with self.val_summary_writer_C.as_default():
            ...
            for param in self.physical_params:
                tf.summary.scalar('Regression_Masked_RMSE_'+param, self.log_val_central['Pred_'+param+'_'+'Masked_RMSE'], step=self.epoch)
                tf.summary.scalar('Regression_Masked_R2_'+param, self.log_val_central['Pred_'+param+'_'+'Masked_R2'], step=self.epoch)
    ...
    def test_loop(self, print_report=True):
        ...
        for batch in self.dataset_test:
            ...
            # Get the regression results, if aplicable
            if 'regression' in self.mode:
                for param in self.physical_params:
        ...
        # Predict
        output = {}
        output['Probability'] = np.array(prob)
        ...
        mask_value = {} # dict to store the values representing the mask.
        if 'regression' in self.mode:
            # Load the scalers
            with open(self.path_scalers, 'rb') as file:
                self.scalers = pickle.load(file)
            ...
            for param in self.physical_params:
                ...
                output['Pred_'+param] = self.scalers[param].inverse_transform(phys_params['Pred_'+param].reshape(-1,1))
        ...
        if print_report:
            ...
            if 'regression' in self.mode:
                ...
                for param in self.physical_params:
                    ...
                    self.regression_scores['RMSE'][param] = mean_squared_error(output[param][mask], output['Pred_'+param][mask], squared=False) #False means RMSE
    ...
    def __add_callbacks(self):
       ...
       # Initialize callbacks for all the models
        ...
        for b in range(self.n_bands):
            ...
            profiler = tf.keras.callbacks.TensorBoard(log_dir = self.log_folder_train,
                                                      histogram_freq = -1,
                                                      profile_batch = '10,12',
                                                      embeddings_freq = 0,
                                                      write_graph=False)
    ...
    def save_setup(self):
        '''Save the dictionaries and metadatas to reconstruct the model for test.'''
